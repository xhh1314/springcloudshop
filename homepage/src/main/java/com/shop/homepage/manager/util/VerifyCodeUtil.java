...
public class VerifyCodeUtil{  
    ...
    /**
     * 传递一个vefifyCode进去，执行reseponse输出图片
     * @param response
     * @return
     */
    @SuppressWarnings("finally")
	public static String excuteVericode(String verifyCode,HttpServletResponse response){
    	int w = 80, h = 32; 
    	//String verifyCode = generateVerifyCode(4); 
    	OutputStream out=null;
    	try {
			out=response.getOutputStream();
			outputImage(w,h,out,verifyCode);//执行打印操作
		} catch (IOException e) {
   ...
   /* public static void main(String[] args) throws IOException{  
        File dir = new File("D:/verifies/code");  
        int w = 200, h = 80; 
        for(int i = 0; i < 50; i++){  
        	String verifyCode = generateVerifyCode(4); 
