...
$("img.selectOne").each(function(){
   ...
   if(state){
	$(".selectAll").each(function(){
		...
		$(this).attr("src","${ctx}/static/image/selected.png");
	});
		...
		$(".selectAll").each(function(){
			...
			$(this).attr("src","${ctx}/static/image/notSelected.png");
...
$(".createOrderButton").click(function(){
   ...
   window.location.href="${ctx}/forePermission/showOrder?"+params;
...
function deleteItem(obj){
	...
	$.ajax({
		url:"${ctx}/forePermission/deleteItem/"+oid,
		method:"post",
		dataType:"text",
...
<div id="noOrderItems"  class="noOrderItems" style="display: none"
		...
		<h2>
			你还没有添加任何商品到购物车<a href="${ctx}/fore/index">继续购物</a>
		</h2>
</div>
<div class="cartDiv">
		<table class="cartProductList">
			<thead>
				<tr>
					<td class="selectAndImage"><img src="${ctx}/static/image/notSelected.png" class="selectAll" selectit="false" > 全选
					</td>
					<td>商品信息</td>
					<td>单价</td>
					<td>数量</td>
			...
			<tbody>
				<c:forEach items="${orderItems}" var="orderItem">
					<tr class="cartProductItemTR" oid="${orderItem.id}">
						<td>
						<img src="${ctx}/static/image/notSelected.png"  oid="${orderItem.id}" class="selectOne" selectit="false"> 
	...
	<div class="cartFoot">
		<img src="${ctx}/static/image/notSelected.png"
